...
<?php 
	...
	if ($existCount > 0) { 
		...
		<p>Please contact: <br/><em>(+25) 078 606 9637</em><br/><b>irasaac@gmail.com</b></p>		
...
<?php // SESSION
	...
	//$SessioncompanyId = preg_replace('#[^A-Za-z0-9]#i', '', $_SESSION["CompanyId"]); // filter everything but numbers and letters
	$check_company_id = $db->query("SELECT * FROM users WHERE username='$username'");
	while ($c=mysqli_fetch_array($check_company_id)) {
		$SessioncompanyId = $c['companyId'];
	}
	$getuserName = $db->query("SELECT * FROM users WHERE username='$username' AND companyId='$SessioncompanyId'");
...
<?php // STOCK SETUP
			...
			<tbody>
				<tr class="gradeX">
					...
					<td class="actions">
						...
						&nbsp;&nbsp;&nbsp;
						<button onclick="setuptable()" class="btn btn-primary  btn-custom waves-effect m-b-5"><i class="fa fa-times"></i></button>
	...
	if(isset($_GET['newItemName'])){
		...
		//$SessioncompanyId = $_SESSION["CompanyId"];
		$sqlcheck = $db->query("SELECT kode FROM items WHERE kode='$itemCode' AND companyId='$SessioncompanyId'");
		$countCHeck = mysqli_num_rows($sqlcheck);
		$addedBy = $_SESSION["username"];
		if(!$countCHeck > 0){
			
		$sql = $db->query("INSERT INTO `items`(`companyId`, `itemName`, `kode`, `unit`, `unityPrice`, `inDate`, `addedBy`) 
		VALUES ('$SessioncompanyId', '$itemName', '$itemCode', '$unit', '$unityPrice', now(), '$addedBy')
		")or die(mysqli_error());
		echo'<script>	
	(function(){
	 setuptable();
...
if(isset($_GET['operationNotes']))
{
	...
	$operationStatus = 1;
	// $getqty = $db ->query("SELECT * FROM `transactions` WHERE companyId='$companyId' AND itemCode='$itemCode'");
	$sql = $db->query("INSERT INTO `transactions`
	(`companyId`, `PurchasePrice`, `trUnityPrice`, `qty`, 
	...
	VALUES 
	('$companyId','$unityPrice','$unityPrice','$qty',
	'$itemCode','$operation','$purchaseOrder',
	'$deliverlyNote','$docRefNumber','$customerName',
	'$customerRef','$operationNotes','$operationStatus', now(), '$doneBy')
	")or die(mysqli_error());

	$insertIntoIn = $db->query("INSERT INTO `intransaction`(`companyId`, `trUnityprice`, `qty`, `qtyBefore`, `itemCode`, `doneOn`) VALUES ('$companyId','$unityPrice','$qty','$qty','$itemCode',now())")or die(mysqli_error());
...
WHERE `purchaseOrder`='$purchaseOrder' AND `deliverlyNote` = '$deliverlyNote' AND `operation` = 'in' AND T.`companyId` = '$SessioncompanyId'")or die(mysqli_error());
	$countout = mysqli_num_rows($sql);
	if($countout > 0)
	{
		$n=0;
...
{
//insert new serie
	$sql1 = $db->query("INSERT INTO `serieid` (`companyId`, `userOn`) VALUES ('$SessioncompanyId','$username')")or die(mysqli_error());	
//get new SN		
...
if(isset($_GET['InvoiceDeliverlyNote']))
{
	...
	// Use FIFO type in  transaction
	$selectFi = $db ->query("SELECT * FROM intransaction WHERE itemCode = '$itemCode' AND qty > 0 LIMIT 1");
	$thisItem = mysqli_fetch_array($selectFi);
	$itemQty = $thisItem['qty'];
	$trID = $thisItem['transactionID'];
	$itemPurchasePrice = $thisItem['trUnityprice'];
	if ($itemQty == $qty) {
		$sql = $db->query("INSERT INTO `transactions`
		(`companyId`, `PurchasePrice`, `trUnityPrice`, `qty`, 
		`itemCode`, `operation`, `purchaseOrder`,
		...
		VALUES 
		('$SessioncompanyId','$itemPurchasePrice','$unityPrice','$itemQty',
		'$itemCode','$operation','$purchaseOrder',
		'$deliverlyNote','$docRefNumber','$customerName',
		'$customerRef','$operationNotes','$operationStatus','$operationNotes', '$doneBy')
		")or die(mysqli_error());
		if ($sql) {
			$updateTransaction = $db ->query("UPDATE intransaction SET qty = 0 WHERE itemCode = '$itemCode' AND transactionID = '$trID'");
		}
	}
	elseif ($itemQty > $qty) {
		$itemQty = $itemQty - $qty;
		$sql = $db->query("INSERT INTO `transactions`(`companyId`, `PurchasePrice`, `trUnityPrice`, `qty`, `itemCode`, `operation`, `purchaseOrder`,`deliverlyNote`, `docRefNumber`, `customerName`, `customerRef`, `operationNotes`, `operationStatus`, `description`, doneBy) VALUES ('$SessioncompanyId','$itemPurchasePrice','$unityPrice','$qty','$itemCode','$operation','$purchaseOrder','$deliverlyNote','$docRefNumber','$customerName','$customerRef','$operationNotes','$operationStatus','$operationNotes', '$doneBy')")or die(mysqli_error());
		if ($sql) {
			$updateTransaction = $db ->query("UPDATE intransaction SET qty = '$itemQty' WHERE itemCode = '$itemCode' AND transactionID = '$trID'");
		}
		
	}
	else {
		$sql = $db->query("INSERT INTO `transactions`(`companyId`, `PurchasePrice`, `trUnityPrice`, `qty`, `itemCode`, `operation`, `purchaseOrder`,`deliverlyNote`, `docRefNumber`, `customerName`, `customerRef`, `operationNotes`, `operationStatus`, `description`, doneBy) VALUES ('$SessioncompanyId','$itemPurchasePrice','$unityPrice','$itemQty','$itemCode','$operation','$purchaseOrder','$deliverlyNote','$docRefNumber','$customerName','$customerRef','$operationNotes','$operationStatus','$operationNotes', '$doneBy')")or die(mysqli_error());
		if ($sql) {
			$updateTransaction = $db ->query("UPDATE intransaction SET qty = 0 WHERE itemCode = '$itemCode' AND transactionID = '$trID'");
		}
		$qty = $qty - $itemQty;
		while ($qty > 0) {
			$selectFi = $db ->query("SELECT * FROM intransaction WHERE itemCode = '$itemCode' AND qty > 0");
			$thisItem = mysqli_fetch_array($selectFi);
			$itemQty = $thisItem['qty'];
			$trID = $thisItem['transactionID'];
			$itemPurchasePrice = $thisItem['trUnityprice'];
